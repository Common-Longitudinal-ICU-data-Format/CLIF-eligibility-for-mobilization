...
if "!CHUNKED!"=="1" (
    for %%Y in (2018 2019 2020 2021 2022 2023 2024) do (
        set "COHORT_YEAR=%%Y"
        set STEP_CMD=uv run --project "!PROJECT_ROOT!" python 01_cohort_identification.py
        set "STEP_NAME=01 Cohort %%Y"
        call :run_step
    )
    set "COHORT_YEAR="
    set STEP_CMD=uv run --project "!PROJECT_ROOT!" python 01b_aggregate_yearly.py
    set "STEP_NAME=01b Aggregate Yearly"
    call :run_step
) else (
    set STEP_CMD=uv run --project "!PROJECT_ROOT!" python 01_cohort_identification.py
    set "STEP_NAME=01 Cohort Identification"
    call :run_step
)

REM Step 2: Mobilization Analysis
set STEP_CMD=uv run --project "!PROJECT_ROOT!" python 02_mobilization_analysis.py
set "STEP_NAME=02 Mobilization Analysis"
call :run_step
...
if errorlevel 1 (
    ...
    set "STEP_CMD=Rscript --vanilla 03_combined_analysis.R"
    set "STEP_NAME=03 Combined R Analysis"
    call :run_step
    set "STEP_CMD=Rscript --vanilla 04_sensitivity_forest_plots.R"
    set "STEP_NAME=04 Sensitivity Forest Plots"
    call :run_step
)
...
!STEP_CMD! > "!STEP_LOG!" 2>&1
if errorlevel 1 (
    call :log "  FAILED - see !STEP_LOG!"
